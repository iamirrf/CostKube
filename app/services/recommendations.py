...
class RecommendationService:
    ...
    def analyze_namespace(
        ...
        # CPU analysis
        cpu_utilization = current_usage['cpu_mcores'] / max(requested_resources['cpu_mcores'], 1)

        if cpu_utilization < self.low_cpu_threshold:
            recommended_cpu = max(current_usage['cpu_mcores'] * 1.3, 100)  # 30% headroom, min 100m
            cpu_savings = (requested_resources['cpu_mcores'] - recommended_cpu) / 1000 * 0.031 * 730
        ...
        # Memory analysis
        memory_utilization = current_usage['memory_bytes'] / max(requested_resources['memory_bytes'], 1)

        if memory_utilization < self.low_memory_threshold:
            recommended_memory = max(current_usage['memory_bytes'] * 1.3, 128 * 1024 * 1024)  # 30% headroom, min 128Mi
            memory_savings = (requested_resources['memory_bytes'] - recommended_memory) / (1024**3) * 0.004 * 730
        ...
        return {
            ...
            'medium_severity_count': sum(1 for r in recommendations if r['severity'] == 'medium')
        }
    ...
    def analyze_all_namespaces(
        ...
        return {
            ...
            'high_priority_count': sum(r['high_severity_count'] for r in all_recommendations),
            'medium_priority_count': sum(r['medium_severity_count'] for r in all_recommendations),
